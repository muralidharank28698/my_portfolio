export const AllData = {
  followMePage: {
    followMePageTitle: 'Follow here',
    followLinksButtonText: 'Click here',
    followMeTitle: { github: 'Github', linkedIn: 'LinkedIn', naukri: 'Naukri' },
    followMeDescription: {
      githubDes:
        'Discover more than 20 projects on my GitHub profile, showcasing 1.5 years of software development experience. To gain a deeper understanding of my skills, follow me on GitHub.',
      linkedInDes:
        'With 3+ years on LinkedIn, connect with me there for direct communication and to explore my professional journey. Follow me on LinkedIn to stay updated on my career and accomplishments.',
      naukriDes:
        'Having maintained a Naukri profile for over 2+ years, you can connect with me there to initiate direct communication. Follow my Naukri profile to stay informed about my career progress and reach out easily.',
    },
    Urls: {
      github: 'https://github.com/muralidharank28698',
      linkedIn: 'https://www.linkedin.com/in/muralidharank280698/',
      naukri: 'https://www.naukri.com/mnjuser/profile?id=&altresid',
    },
  },
  header: {
    mailId: 'muralidharank28698@gmail.com',
    phoneNO: 'PhoneNo: 8098633412',
    phoneNoOnly: '8098633412',
  },
  leftSideData: {
    name: 'Muralidharan K',
    role: 'Software Developer',
    languageKnown: {
      tamil: 'Tamil',
      english: 'English',
    },
    skillsAndAbilities:
      'Ability to work under pressure, best decision making, time-management, self-motivation, conflict resolution, leadership and adapatabiliy.',
    tableData: {
      residence: 'Puducherry',
      city: 'Puducherry',
      age: '25',
    },
  },
  certerPart: {
    objective:
      'Experienced Software Developer with 1.5 years of hands-on expertise in the dynamic and ever-evolving field of software development. Proficient in designing, developing, and deploying software solutions that drive efficiency and innovation. Demonstrated ability to collaborate effectively within cross-functional teams to deliver high-quality code and meet project objectives. Continuously staying up-to-date with the latest industry trends and technologies to ensure the delivery of cutting-edge solutions. Excited to contribute my skills and experience to create impactful software projects and solutions in the future.',
    role: 'I am a software developer..!',
    reactDeveloper: 'React Developer',
    experience: '01+ years experience',
    projects: '10+ projects',
    projectDescription: 'have been developed',
    hackerRank: 'Hackerrank profile',
    hackerRankDes: '80% score in hacker rank portal',
    deployProjects: '02+ projects',
    deployProjectsDes: '02+ projects have deployed',
  },
};
